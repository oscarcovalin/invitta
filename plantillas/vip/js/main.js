...
const savedConfig = localStorage.getItem('wedding_config_machote');
if (savedConfig) {
    try {
        activeConfig = JSON.parse(savedConfig);
        console.log("Cargada configuración de boda desde localStorage del navegador.");
    } catch (e) {
        console.error("Error leyendo localStorage, usando fallback.", e);
    }
}

// Fallback: Si no hay datos en localStorage, usar el archivo estático config.js
if (!activeConfig) {
    if (typeof WEDDING_CONFIG !== "undefined") {
        activeConfig = WEDDING_CONFIG;
        console.log("Cargada configuración de boda estática desde js/config.js.");
    } else {
        console.error("No se detectó ningún objeto de configuración.");
    }
}
...
if (activeConfig && activeConfig.theme) {
    ...
    if (theme) {
        ...
        if (theme.bgImage && theme.bgImage.trim() !== "") {
            ...
            root.style.setProperty('--global-bg-image', `url('../assets/section_bg.png')`);
        }
    }
