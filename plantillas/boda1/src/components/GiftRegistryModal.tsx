...
import { ShoppingBag, Gift, Landmark, Mail } from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="text-center space-y-4">
          <div className="flex items-center justify-center text-sage">
            {registry.id === "amazon" && <ShoppingBag className="w-10 h-10 stroke-[1.5]" />}
            {registry.id === "liverpool" && <Gift className="w-10 h-10 stroke-[1.5]" />}
            {registry.id === "bank" && <Landmark className="w-10 h-10 stroke-[1.5]" />}
            {registry.id === "envelopes" && <Mail className="w-10 h-10 stroke-[1.5]" />}
          </div>
        ...
        <div className="font-sans text-on-surface-variant/90 text-[15px] leading-relaxed text-center space-y-6">
            ...
            <div className="bg-surface-container-low/60 p-6 border border-outline-variant/20 space-y-4 text-left">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center justify-between gap-4 mt-1">
                  <p className="font-mono text-xs md:text-sm text-ink tracking-wider font-semibold select-all bg-surface-container-low px-2 py-1 border border-outline-variant/30 rounded-sm">
                    {registry.clabe}
                  </p>
                  <button 
