...
<style>
  ...
  :root {
    ...
    --scale-hero: ${Math.max(0.7, Math.min(1.4, Number((config.typography && config.typography.scaleHero) || 1.0)))};
    --scale-headings: ${Math.max(0.7, Math.min(1.3, Number((config.typography && config.typography.scaleHeadings) || 1.0)))};
    --scale-body: ${Math.max(0.8, Math.min(1.25, Number((config.typography && config.typography.scaleBody) || 1.0)))};
  ...
  /* Clases con Escala Tipográfica Controlada y Anti-Desborde */
  .hero-name-scaled {
    font-size: calc(clamp(2.75rem, 8.5vw, 5.5rem) * var(--scale-hero)) !important;
  }
  .hero-solo-name-scaled {
    font-size: calc(clamp(3.25rem, 10vw, 6.5rem) * var(--scale-hero)) !important;
  }
  .hero-connector-scaled {
    font-size: calc(clamp(1.75rem, 5vw, 3rem) * var(--scale-hero)) !important;
  }
  .heading-script-scaled {
    font-size: calc(clamp(2.25rem, 6.5vw, 4.25rem) * var(--scale-headings)) !important;
  }
  .heading-display-scaled {
    font-size: calc(clamp(1.5rem, 4vw, 2.25rem) * var(--scale-headings)) !important;
  }
  .body-scaled {
    font-size: calc(1rem * var(--scale-body)) !important;
  }
