...
<div class="flex flex-col w-full overflow-hidden relative">
  ...
  <section id="hero" class="relative ${hasHeroPhoto ? 'pt-0 pb-12' : 'min-h-[68vh] sm:min-h-[72vh] flex flex-col items-center justify-center pt-10 pb-6 px-margin-mobile'} text-center" style="overflow: visible;">
    ...
    <!-- ╚══════════════════════════════════════════════════════════════╝ -->
    <div id="heroPhotoFrame" class="w-full relative min-h-[55vh] max-h-[88vh] overflow-hidden mb-6 flex items-center justify-center bg-black/5">
      <div id="parallaxBgHero" class="w-full h-full will-change-transform flex items-center justify-center">
        <img id="heroPhotoImg" src="${config.photos.hero}" alt="${safeTitle}" class="w-full h-auto max-h-[88vh] object-contain block mx-auto rounded-none" loading="eager"/>
      </div>
    </div>
  ...
  <section id="galeria" class="py-16 px-margin-mobile max-w-xl mx-auto relative overflow-hidden ${isGalleryEnabled ? '' : 'hidden'}">
    ${(() => {
      ...
      return (bg.image && bg.image.trim()) ? `
        <div id="parallaxBgGallery" class="absolute inset-0 w-full h-[120%] -top-[10%] z-0 pointer-events-none will-change-transform" style="opacity: ${bg.opacity};">
          <div class="w-full h-full bg-cover bg-center" style="background-image: url('${bg.image}');"></div>
        </div>
      ` : '';
    })()}
    <div class="relative z-10 w-full">
      ...
      <!-- Galería Editorial de Cuadros Fotográficos Completos Sin Cortes y Sin Esquinas Redondeadas -->
      <div class="w-full flex flex-col gap-6 sm:gap-8" id="galleryGrid">
        ${((config.photos && config.photos.gallery) || []).map((url, i) => url ? `
          <div class="gallery-item-wrap w-full relative rounded-none bg-white/50 backdrop-blur-sm p-1 sm:p-2 border border-antique-gold/40 shadow-[0_10px_35px_rgba(165,110,14,0.08)]">
            <div class="w-full relative overflow-hidden rounded-none bg-black/5">
              <img src="${url}" alt="Recuerdo ${i+1}" loading="lazy" class="gallery-photo w-full h-auto block object-contain mx-auto rounded-none transition-transform duration-700 hover:scale-[1.01]"/>
            </div>
          </div>
        ` : '').join('')}
      </div>
...
document.addEventListener("DOMContentLoaded", () => {
  if(typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined'){
    ...
    if (heroSection && parallaxBgHero) {
      ...
      gsap.to(parallaxBgHero, {
        ...
        scrollTrigger: {
          ...
          scrub: 0.6   // más responsivo e inmediato
        }
      });
    }
    ...
    if (illBridgeHero && !illBridgeHero.classList.contains('hidden')) {
      ...
      const heroParallaxSpeed = ${Number(illHero.parallaxSpeed !== undefined ? illHero.parallaxSpeed : 30)};
    ...
    if (illBridgeCountdown && !illBridgeCountdown.classList.contains('hidden')) {
      ...
      const cdParallaxSpeed = ${Number(illCountdown.parallaxSpeed !== undefined ? illCountdown.parallaxSpeed : 30)};
    ...
    if (familySection && illBridgeFamily && !illBridgeFamily.classList.contains('hidden')) {
      ...
      const familyParallaxSpeed = ${Number(illFamily.parallaxSpeed !== undefined ? illFamily.parallaxSpeed : 30)};
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // PARALLAX EN GALERÍA DE RECUERDOS (Efecto de Desplazamiento Fluido Sin Cortes)
    // ─────────────────────────────────────────────────────────────────────────
    const galleryWraps = document.querySelectorAll('.gallery-item-wrap');
    ...
    const parallaxBgGallery = document.getElementById('parallaxBgGallery');
    if (parallaxBgGallery) {
      gsap.to(parallaxBgGallery, {
        yPercent: -35,
        ease: 'none',
        scrollTrigger: {
          trigger: document.getElementById('galeria'),
          start: 'top bottom',
          end: 'bottom top',
          scrub: 0.6
        }
