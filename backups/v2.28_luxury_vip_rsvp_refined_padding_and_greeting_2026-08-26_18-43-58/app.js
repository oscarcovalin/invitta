...
// ==================== VIP PASS SIMULATOR & PREVIEW ====================
function setupVipPassGenerator() {
  const simButtons = document.querySelectorAll('.btn-simulate-vip');
  simButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const passes = btn.dataset.passes || '';
      ...
      let paramStr = '';
      if (guest) {
        paramStr = `?guest=${encodeURIComponent(guest)}&passes=${passes}&mesa=${mesa}`;
      }
      ...
      iframe.onload = () => {
        try {
          ...
          const rsvpEl = doc.getElementById('rsvp');
          if (rsvpEl) {
      ...
      showToast(guest ? `Simulando: ${guest} (${passes} pases · Mesa ${mesa})` : 'Simulando: Vista General');
    });
  });
}
