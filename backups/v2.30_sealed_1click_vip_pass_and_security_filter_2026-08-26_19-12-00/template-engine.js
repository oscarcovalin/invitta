...
<style>
  ...
  /* ==================== ESTILO PERGAMINO RSVP ==================== */
  .pergamino-container {
    background: #ffffff;
    border: 2px solid var(--gold);
    border-radius: 20px;
    padding: 38px 28px;
    box-shadow: 0 18px 45px rgba(22, 60, 43, 0.18), inset 0 0 35px rgba(165, 110, 14, 0.04);
    position: relative;
  }
  .pergamino-ornament {
    display: flex;
  ...
  .mensaje-personalizado-pergamino {
    background-color: rgba(165, 110, 14, 0.08);
...
<div class="flex flex-col w-full overflow-hidden relative">
  ...
  <!-- ==================== 10. RSVP PERGAMINO (CONFIRMACIÓN ULTRA-SIMPLE) ==================== -->
  <section id="rsvp" class="pt-16 pb-36 px-margin-mobile bg-emerald-dark text-center relative overflow-hidden border-t border-antique-gold/30 ${isRsvpEnabled ? '' : 'hidden'}">
    ...
    <div class="relative z-10 max-w-md mx-auto gsap-scale-up overflow-visible">
      ...
      <!-- Tarjeta Pergamino Minimalista y Elegante -->
      <div class="pergamino-container text-deep-onyx" id="pergaminoCard">
        ...
        <h2 class="font-display-lg text-2xl sm:text-3xl font-normal text-deep-onyx mb-1 tracking-wide uppercase">
          Confirmación de Asistencia
        </h2>
        <p class="font-display-lg italic text-sm sm:text-base text-antique-gold mb-4 font-light" id="subEventoRsvp">
          ${fullDisplayName} · ${config.eventDateShort || config.eventDateLabel || ''}
        </p>

        <!-- Saludo Personalizado VIP / Tarjeta de Honor -->
        <div id="contenedor-saludo-pergamino" class="mb-4"></div>

        <!-- Formulario RSVP Ultra-Simple -->
        <form id="rsvpForm" class="flex flex-col gap-4 text-left" novalidate>
          
          <!-- Nombre (solo si no viene por URL) -->
          <div id="campoNombrePergamino" class="flex flex-col gap-1">
            <label class="font-label-caps text-[10.5px] text-deep-onyx uppercase tracking-[0.2em] font-semibold">
              Tu Nombre y Apellidos
            </label>
            <input id="formGuestName" name="nombre" required class="w-full bg-[#FAF8F5] border border-antique-gold/40 rounded-xl px-4 py-2.5 font-body-lg text-deep-onyx focus:outline-none focus:border-antique-gold text-sm placeholder:text-neutral-400" placeholder="Escribe tu nombre..." type="text"/>
          </div>

          <div id="campoTelefonoPergamino" class="flex flex-col gap-1">
            <label class="font-label-caps text-[10.5px] text-deep-onyx uppercase tracking-[0.2em] font-semibold">
              Teléfono / WhatsApp
            </label>
            <input id="formGuestPhone" name="telefono" class="w-full bg-[#FAF8F5] border border-antique-gold/40 rounded-xl px-4 py-2.5 font-body-lg text-deep-onyx focus:outline-none focus:border-antique-gold text-sm placeholder:text-neutral-400" placeholder="Número de celular" type="tel"/>
          </div>

          <!-- Selector de Asistencia con 2 Grandes Botones -->
          <div class="flex flex-col gap-2">
            <label class="font-label-caps text-[10.5px] text-deep-onyx uppercase tracking-[0.2em] font-semibold text-center block">
              ¿Nos acompañarán?
            </label>
            <div class="grid grid-cols-2 gap-2.5">
              <label id="labelAsistenciaSi" class="flex flex-col items-center justify-center gap-1 cursor-pointer p-3.5 border-2 border-antique-gold rounded-2xl bg-antique-gold/10 hover:bg-antique-gold/20 transition-all text-center">
                <input class="hidden" name="rsvp" value="si" checked type="radio" id="radioAsistenciaSi"/>
                <span class="text-xl">✨</span>
                <span class="font-body-lg text-xs font-bold text-deep-onyx">¡Sí, asistiremos!</span>
              </label>
              <label id="labelAsistenciaNo" class="flex flex-col items-center justify-center gap-1 cursor-pointer p-3.5 border border-neutral-300 rounded-2xl bg-white/50 hover:bg-neutral-100 transition-all text-center opacity-70">
                <input class="hidden" name="rsvp" value="no" type="radio" id="radioAsistenciaNo"/>
                <span class="text-xl">💌</span>
                <span class="font-body-lg text-xs font-medium text-neutral-600">No podré asistir</span>
              </label>
            </div>
          </div>

          <!-- Selector de Pases con Pastillas / Chips Rápidos -->
          <div id="rsvpTicketsRow" class="flex flex-col gap-2 p-3 bg-champagne-cream/60 border border-antique-gold/30 rounded-2xl">
            <div class="flex items-center justify-between">
              <span class="font-label-caps text-[10.5px] text-deep-onyx uppercase tracking-[0.2em] font-semibold">¿Cuántos asistirán?</span>
              <span id="allowedTicketsHint" class="text-[11px] text-antique-gold font-bold">2 Pases</span>
            </div>
            <div id="pillsContainer" class="flex flex-wrap gap-2 justify-center pt-1">
              <!-- Se generan dinámicamente según urlTickets (1, 2, 3, 4...) -->
            </div>
          </div>

          <!-- Campo Opcional de Mensaje / Dieta -->
          <div id="rsvpDietaryBlock" class="flex flex-col gap-1 pt-1">
            <label class="font-label-caps text-[10px] text-tertiary uppercase tracking-[0.2em]">
              💬 Mensaje o dieta especial (Opcional)
            </label>
            <input type="text" name="alergias" id="formGuestAllergies" placeholder="Ej: 1 vegetariano, felicitaciones..." 
                   class="w-full bg-[#FAF8F5] border border-antique-gold/30 rounded-xl px-3.5 py-2.5 text-xs text-deep-onyx focus:outline-none focus:border-antique-gold placeholder:text-neutral-400">
          </div>

          <!-- Botón de Confirmación Principal -->
          <button type="submit" id="rsvpSubmit" class="btn-luxury-primary w-full py-4 text-xs tracking-[0.2em] uppercase font-bold shadow-[0_4px_20px_rgba(165,110,14,0.35)] mt-1 cursor-pointer transition-all hover:scale-[1.01] active:scale-[0.99] flex items-center justify-center gap-2">
        ...
        <div id="rsvpSuccess" class="hidden text-center py-2">
          ...
          <div class="flex flex-col sm:flex-row gap-2 justify-center mb-3">
            <button type="button" id="btnSharePassWhatsapp" class="btn-luxury-primary text-[11px] py-2 px-4 uppercase tracking-wider font-semibold inline-flex items-center justify-center gap-1.5">
              <span>📲 Reenviar por WhatsApp</span>
            </button>
            <button type="button" id="btnDownloadPassImage" class="btn-luxury-secondary text-[11px] py-2 px-4 uppercase tracking-wider font-semibold inline-flex items-center justify-center gap-1.5">
              <span>💾 Guardar Pase</span>
            </button>
          </div>

          <button type="button" id="btnResetRsvp" class="text-xs text-antique-gold hover:underline font-medium cursor-pointer">
            ↺ Modificar respuesta
          </button>
        </div>
...
// 6. RSVP Pergamino: Detección, Stepper, Folio Logístico, Dietas y QR Pass
(function initRsvpPergamino() {
  const saludoCont = document.getElementById('contenedor-saludo-pergamino');
  const campoNombre = document.getElementById('campoNombrePergamino');
  const inputNombre = document.getElementById('formGuestName');
  const guestOut = document.getElementById('guestCount');
  const ticketsRow = document.getElementById('rsvpTicketsRow');
  const ticketsHint = document.getElementById('allowedTicketsHint');
  const dietaryBlock = document.getElementById('rsvpDietaryBlock');
  ...
  let guestCount = Math.min(urlTickets, Math.max(1, urlTickets));
  const urlMesaParam = urlParams.get('mesa') || urlParams.get('table') || urlParams.get('m') || '';

  const pillsContainer = document.getElementById('pillsContainer');
  const labelAsistenciaSi = document.getElementById('labelAsistenciaSi');
  const labelAsistenciaNo = document.getElementById('labelAsistenciaNo');
  const radioSi = document.getElementById('radioAsistenciaSi');
  const radioNo = document.getElementById('radioAsistenciaNo');

  let guestCount = Math.max(1, urlTickets || 2);
  const urlMesaParam = urlParams.get('mesa') || urlParams.get('table') || urlParams.get('m') || '';
  const isVipGuest = !!(urlGuest && urlGuest.trim());

  // Render Pill Chips for Pass Selection
  const renderPillChips = (maxPasses) => {
    if (!pillsContainer) return;
    pillsContainer.innerHTML = '';
    const total = Math.max(1, Math.min(12, maxPasses));
    for (let i = 1; i <= total; i++) {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'pill-ticket-btn px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer select-none ' + 
        (i === guestCount 
          ? 'bg-antique-gold text-white shadow-md scale-105 border border-antique-gold' 
          : 'bg-white/80 border border-antique-gold/30 text-deep-onyx hover:bg-champagne-cream');
      btn.textContent = i === total && total > 1 ? (i + ' (Todos)') : (i + (i === 1 ? ' Pase' : ' Pases'));
      btn.onclick = () => {
        guestCount = i;
        renderPillChips(maxPasses);
      };
  ...
  if (isVipGuest) {
    let mesaLabelTxt = '';
    if (urlMesaParam) {
      mesaLabelTxt = urlMesaParam.toLowerCase().includes('imp') ? ' en la <strong class="text-primary font-bold">Mesa Imperial</strong>' : (' en la <strong class="text-primary font-bold">Mesa ' + urlMesaParam.replace(/\\D/g, '') + '</strong>');
    }

    if (saludoCont) {
      saludoCont.innerHTML = '<div class="mensaje-personalizado-pergamino shadow-sm border border-antique-gold/40 rounded-2xl bg-amber-500/10 p-4 mb-2 text-left">' +
        '<div class="flex items-center gap-2 mb-1 text-antique-gold font-bold">' +
          '<span class="text-xl">⚜️</span>' +
          '<strong class="font-display-lg text-xs uppercase tracking-wider text-deep-onyx">Pase de Honor Pre-Asignado</strong>' +
        '</div>' +
        '<p class="text-sm text-deep-onyx leading-relaxed">' +
          '¡Qué alegría tenerles aquí, <strong class="text-primary font-bold text-base">' + urlGuest + '</strong>!<br>' +
          'Tienen <strong class="text-primary font-bold">' + urlTickets + (urlTickets === 1 ? ' pase reservado' : ' pases reservados') + '</strong>' +
          mesaLabelTxt + ' para celebrar con nosotros.' +
        '</p>' +
      '</div>';
    }
    if (campoNombre) campoNombre.style.display = 'none';
    if (document.getElementById('campoTelefonoPergamino')) document.getElementById('campoTelefonoPergamino').style.display = 'none';
    if (inputNombre) inputNombre.value = urlGuest;
    if (ticketsHint) ticketsHint.textContent = urlTickets + ' Pase' + (urlTickets === 1 ? '' : 's');
    if (btnSubmit) btnSubmit.innerHTML = '<span>✨ Confirmar Asistencia y Obtener Pase QR</span>';
  } else {
    if (saludoCont) {
      saludoCont.innerHTML = '<div class="mensaje-personalizado-pergamino shadow-sm border border-antique-gold/30 rounded-2xl bg-champagne-cream/50 p-3.5 mb-2 text-left">' +
        '<div class="flex items-center gap-1.5 mb-0.5 text-antique-gold font-bold">' +
          '<span class="text-base">⚜️</span>' +
          '<strong class="font-display-lg text-[11px] uppercase tracking-wider text-deep-onyx">Solicitud de Registro</strong>' +
        '</div>' +
        '<p class="text-xs text-deep-onyx/85 leading-relaxed">' +
          'Para proteger el acomodo del salón, ingresa tus datos. Los novios te asignarán tu mesa oficial y recibirás tu pase QR definitivo.' +
        '</p>' +
      '</div>';
    }
    if (campoNombre) campoNombre.style.display = 'flex';
    if (document.getElementById('campoTelefonoPergamino')) document.getElementById('campoTelefonoPergamino').style.display = 'flex';
    if (inputNombre) inputNombre.value = '';
    if (ticketsHint) ticketsHint.textContent = urlTickets + ' Pase' + (urlTickets === 1 ? '' : 's');
    if (btnSubmit) btnSubmit.innerHTML = '<span>📲 Enviar Solicitud a los Novios</span>';
  }

  // Render pills
  renderPillChips(urlTickets);

  // Quick 1-touch Attendance selection
  if (labelAsistenciaSi && labelAsistenciaNo && radioSi && radioNo) {
    labelAsistenciaSi.onclick = () => {
      radioSi.checked = true;
      labelAsistenciaSi.className = 'flex flex-col items-center justify-center gap-1 cursor-pointer p-3.5 border-2 border-antique-gold rounded-2xl bg-antique-gold/15 transition-all text-center';
      labelAsistenciaNo.className = 'flex flex-col items-center justify-center gap-1 cursor-pointer p-3.5 border border-neutral-300 rounded-2xl bg-white/50 hover:bg-neutral-100 transition-all text-center opacity-60';
      if (ticketsRow) ticketsRow.style.display = 'flex';
      if (dietaryBlock) dietaryBlock.style.display = 'flex';
      if (btnSubmit) btnSubmit.innerHTML = isVipGuest ? '<span>✨ Confirmar Asistencia y Obtener Pase QR</span>' : '<span>📲 Enviar Solicitud a los Novios</span>';
    };

    labelAsistenciaNo.onclick = () => {
      radioNo.checked = true;
      labelAsistenciaNo.className = 'flex flex-col items-center justify-center gap-1 cursor-pointer p-3.5 border-2 border-neutral-600 rounded-2xl bg-neutral-200 transition-all text-center';
      labelAsistenciaSi.className = 'flex flex-col items-center justify-center gap-1 cursor-pointer p-3.5 border border-neutral-300 rounded-2xl bg-white/50 hover:bg-neutral-100 transition-all text-center opacity-60';
      if (ticketsRow) ticketsRow.style.display = 'none';
      if (dietaryBlock) dietaryBlock.style.display = 'none';
      if (btnSubmit) btnSubmit.innerHTML = '<span>💌 Enviar Aviso (No podré asistir)</span>';
    };
  }
  ...
  if (btnReset) {
    btnReset.addEventListener('click', () => {
      ...
      if (btnSubmit) {
        ...
        btnSubmit.innerHTML = isVipGuest ? '<span>✨ Confirmar Asistencia y Obtener Pase QR</span>' : '<span>📲 Enviar Solicitud a los Novios</span>';
      }
      if (urlGuest && inputNombre) inputNombre.value = urlGuest;
      if (ticketsRow) ticketsRow.style.display = 'flex';
      if (dietaryBlock) dietaryBlock.style.display = 'flex';
      guestCount = Math.max(1, urlTickets);
      renderPillChips(urlTickets);
    });
  }
  ...
  if (rsvpForm) {
    rsvpForm.addEventListener('submit', (e) => {
      ...
      const isAttending = data.get('rsvp') === 'si';
      const guestName = (data.get('nombre') || urlGuest || 'Invitado de Honor').trim();
      const phone = (data.get('telefono') || '').trim();
      const confirmedTickets = isAttending ? guestCount : 0;
      const allergies = (data.get('alergias') || '').trim();

      // Generar Folio Logístico Opción B: [MESA]-[APELLIDO]-[PASES]
      ...
      const urlMesa = urlParams.get('mesa') || urlParams.get('table') || urlParams.get('m');
      if (urlMesa) {
        if (String(urlMesa).toLowerCase().includes('imp')) {
          mesaCode = 'MIMP';
          tableDisplay = 'Mesa Imperial';
        } else {
          const mMatch = String(urlMesa).match(/\\d+/);
      ...
      const triggerWhatsApp = () => {
        if (CONFIG.whatsappNumber && CONFIG.whatsappNumber.trim()) {
          ...
          const lines = [
            '✨ *Confirmación Pergamino · ' + (CONFIG.eyebrow || 'Boda') + ' de ' + calcDisplayName + '*',
            '👤 *Titular:* ' + guestName,
            '💌 *Estado:* ' + attendLine
          ];
          if (isAttending) {
            lines.push('🎟️ *Pases Confirmados:* ' + confirmedTickets + ' de ' + urlTickets);
            if (isVipGuest) {
              lines.push('🪑 *Mesa Asignada:* ' + tableDisplay);
              lines.push('🔖 *Folio de Acceso:* ' + folio);
            } else {
              lines.push('⏳ *Mesa:* Pendiente de asignación por los Novios');
            }
            if (allergies) lines.push('🍽️ *Nota / Dieta:* ' + allergies);
          }
          const text = encodeURIComponent(lines.join('\\n'));
          const waPhone = CONFIG.whatsappNumber.replace(/[^\\d]/g, '');
      ...
      // Auto-trigger WhatsApp on confirm
      triggerWhatsApp();
        ...
        if (rsvpSuccess) {
          ...
          if (isAttending) {
            if (isVipGuest) {
              ...
              if (msgEl) msgEl.textContent = '¡Pase Digital Sellado! Presenta este código QR en la entrada del salón.';
              ...
              // Invitado en Lista de Espera / Sin Mesa Asignada aún
              if (ticketsEl) ticketsEl.textContent = 'Solicitud por ' + confirmedTickets + ' pase' + (confirmedTickets === 1 ? '' : 's');
              if (msgEl) msgEl.textContent = 'Tu solicitud ha sido enviada a los Novios. Te notificarán tu mesa y te enviarán tu Pase QR definitivo.';
              if (badgeTableEl) badgeTableEl.classList.add('hidden');
              const qrContainer = document.getElementById('qrContainer');
              if (qrContainer) qrContainer.innerHTML = '<div class="p-3 bg-amber-500/10 border border-antique-gold/30 rounded-2xl my-2"><span class="text-3xl block mb-1">⏳</span><span class="text-xs font-bold text-deep-onyx">Mesa en proceso de asignación</span></div>';
            }
          } else {
...
(function initSecureCollaborativeAlbum() {
  ...
  // 5. Galería Privada de los Novios
