...
<div class="flex flex-col w-full overflow-hidden relative">
  ...
  <section id="hero" class="relative ${hasHeroPhoto ? 'pt-4 sm:pt-6 pb-12' : 'min-h-[68vh] sm:min-h-[72vh] flex flex-col items-center justify-center pt-10 pb-6'} px-margin-mobile text-center" style="overflow: visible;">
    
    ${hasHeroPhoto ? `
    <!-- ╔══════════════════════════════════════════════════════════════╗ -->
    <!-- ║  FOTOGRAFÍA PRINCIPAL LIMPIA (SIN TEXTO ENCIMA)              ║ -->
    <!-- ╚══════════════════════════════════════════════════════════════╝ -->
    <div id="heroPhotoFrame" class="w-full max-w-sm sm:max-w-md mx-auto mb-8 px-2 sm:px-0 gsap-scale-up">
      <div class="relative w-full aspect-[3/4] sm:aspect-[4/5] rounded-3xl overflow-hidden shadow-[0_16px_50px_rgba(0,0,0,0.18)] border border-antique-gold/40 bg-black/5">
        <img id="heroPhotoImg" src="${config.photos.hero}" alt="${safeTitle}" class="w-full h-full object-cover object-center transform transition-transform duration-1000 hover:scale-105" loading="eager"/>
      </div>
    </div>
    ...
    <!-- ╚══════════════════════════════════════════════════════════════╝ -->
    <div class="relative z-10 flex flex-col items-center gap-4 max-w-lg mx-auto gsap-fade-up w-full mt-2">
      
      <!-- VIP Guest Banner -->
      <div id="vipBanner" class="hidden flex-col items-center gap-0.5 px-6 py-2 bg-white/80 backdrop-blur-md rounded-full border border-antique-gold/40 shadow-md mb-1">
        ...
        <span id="vipGuestName" class="font-display-lg text-base text-deep-onyx font-bold tracking-wide">Familia Invitada</span>
        <span id="vipGuestTickets" class="font-label-caps text-[10px] text-tertiary font-medium">2 Pases Reservados</span>
      </div>

      <!-- Eyebrow: NUESTRA BODA / MIS XV AÑOS -->
      <p id="heroEyebrow" class="font-label-caps text-xs sm:text-sm text-antique-gold tracking-[0.4em] gsap-eyebrow font-semibold">
        ${config.eyebrow || (isWedding ? 'Nuestra Boda' : 'Mis XV Años')}
      </p>
      ...
      <div class="flex flex-col items-center my-0.5 gsap-title overflow-visible">
        ...
        <span class="font-script text-antique-gold/90 my-0 font-normal leading-normal drop-shadow-sm select-none overflow-visible hero-connector-scaled" id="heroNameConnector" style="font-family: var(--font-script);">
          ${nameConnector}
        </span>
      ...
      <div id="welcomeQuoteWrap" class="mt-1 max-w-[90%] mx-auto ${config.quote ? '' : 'hidden'}">
        <p id="welcomeQuote" class="font-script text-2xl sm:text-3xl md:text-4xl italic leading-relaxed text-deep-onyx/90 font-normal">
      ...
      <div id="welcomeMessageWrap" class="max-w-[460px] mx-auto mt-1 ${config.welcomeMessage ? '' : 'hidden'}">
        <p id="welcomeMessage" class="font-body-lg text-sm sm:text-base text-deep-onyx/85 leading-relaxed font-normal">
    ...
    <div class="relative z-10 flex flex-col items-center gap-4 max-w-lg mx-auto gsap-scale-up w-full mt-2">
      ...
      <!-- Eyebrow: NUESTRA BODA / MIS XV AÑOS -->
      <p id="heroEyebrow" class="font-label-caps text-xs sm:text-sm text-[#f7f6ec] tracking-[0.4em] drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] gsap-eyebrow font-semibold">
        ${config.eyebrow || (isWedding ? 'Nuestra Boda' : 'Mis XV Años')}
      </p>
  ...
  <section id="countdownSection" class="${countdownPaddingTop} pb-20 px-margin-mobile text-center relative max-w-lg mx-auto gsap-fade-up overflow-visible ${isCountdownEnabled ? '' : 'hidden'}">
    ...
    <!-- Se posiciona al pie de la cuenta regresiva, cruzando hacia la sección de Familia y Padrinos -->
  ...
  <section id="dresscode" class="py-20 px-margin-mobile bg-emerald-deep text-center border-y border-antique-gold/30 relative overflow-hidden ${isDressCodeEnabled ? '' : 'hidden'}">
    ...
    <div class="max-w-xl mx-auto relative z-10">
      ...
      <p class="font-label-caps text-[13px] text-antique-gold tracking-[0.3em] mb-6 gsap-eyebrow">${(config.dressCode && config.dressCode.title) || 'Formal'}</p>
      ...
      <div id="dressPaletteWrapper" class="flex flex-wrap justify-center items-center gap-4 sm:gap-6 gsap-fade-up ${(config.dressCode && config.dressCode.colorsEnabled !== false && config.dressCode.colorPalette && config.dressCode.colorPalette.length) ? '' : 'hidden'}">
        ${((config.dressCode && config.dressCode.colorPalette) || []).map(c => `
          <div class="flex flex-col items-center gap-2">
            ...
            <span class="font-label-caps text-[9px] text-[#f7f6ec]/75 tracking-wider">${c.name}</span>
          </div>
        `).join('')}
      ...
      <div class="flex flex-col gap-4 max-w-sm mx-auto gsap-stagger-container mb-8">
        ${((config.giftRegistry && config.giftRegistry.stores) || []).map(store => `
          <a class="bg-white/60 backdrop-blur-sm border border-antique-gold/40 p-5 flex items-center justify-between shadow-sm hover:bg-white/90 hover:shadow-md transition-all gsap-stagger-item rounded-2xl" href="${store.url || '#'}" target="_blank" rel="noopener noreferrer">
            <span class="font-label-caps text-xs text-deep-onyx tracking-widest font-semibold">${store.name}</span>
  ...
  <section id="albumColaborativo" class="py-16 px-margin-mobile text-center max-w-xl mx-auto relative overflow-hidden ${isSharedAlbumEnabled ? '' : 'hidden'}">
    ...
    <div class="bg-white/60 backdrop-blur-xl border border-antique-gold/40 p-8 sm:p-10 rounded-2xl shadow-[0_8px_30px_rgba(165,110,14,0.08)] relative z-10 overflow-hidden gsap-scale-up">
      ...
      <p class="font-label-caps text-xs text-antique-gold tracking-[0.3em] mb-2" id="albumSubtitle">
  ...
  <section id="rsvp" class="py-24 px-margin-mobile bg-emerald-dark text-center relative overflow-hidden border-t border-antique-gold/30 ${isRsvpEnabled ? '' : 'hidden'}">
    ...
    <div class="relative z-10 max-w-md mx-auto gsap-scale-up overflow-visible">
      ...
      <div id="rsvpSuccess" class="hidden mt-6 bg-white text-deep-onyx p-6 sm:p-8 rounded-2xl border-2 border-dashed border-antique-gold shadow-2xl max-w-sm mx-auto text-center">
        ...
        <span class="font-display-lg text-sm tracking-widest text-deep-onyx font-bold block mt-1" id="qrEventTitle">${safeTitle}</span>
...
<footer class="pt-16 pb-24 px-margin-mobile text-center bg-emerald-deep border-t border-antique-gold/30">
  ...
  <p class="font-display-lg text-lg text-[#f7f6ec] tracking-[0.25em] gsap-fade-up font-semibold" id="footerNames">
    ${fullDisplayName}
  </p>
  ...
  <div id="vendorCard" class="mt-12 pt-8 border-t border-antique-gold/20 max-w-sm mx-auto gsap-fade-up ${(vendorCard.enabled !== false) ? '' : 'hidden'}">
    ...
    <span class="font-label-caps text-[9px] text-[#f7f6ec]/40 tracking-widest block mt-3" id="vendorCardAgency">
