...
const TemplateEngine = {
  ...
  defaultConfig: {
    ...
    vendorCard: {
      enabled: true,
      badge: "¿Te gustó esta invitación?",
      title: "Diseña tu Invitación Digital de Lujo",
      description: "Sorprende a tus invitados con una experiencia interactiva inolvidable para Bodas, XV Años y Eventos Exclusivos.",
      whatsappNumber: "5215512345678",
      ...
      showAgencyNote: true,
      agencyName: "Invitaciones Digitales de Lujo"
    }
  },
  ...
  generateHTML(config, themeName = "vino", customTheme = null, decorAssets = {}) {
    ...
    const hasStory = !!(config.story && config.story.enabled !== false && (config.story.title || config.story.text));
    ...
    const countdownPaddingTop = hasIllHero      ? `pt-[calc(140px+${heroPad}px)] sm:pt-[calc(160px+${heroPad}px)]` : 'pt-2 sm:pt-4';
    const familyPaddingTop    = hasIllCountdown ? `pt-[calc(130px+${countdownPad}px)] sm:pt-[calc(150px+${countdownPad}px)] pb-20` : 'py-20';
    const detailsPaddingTop   = hasIllFamily    ? `pt-[calc(130px+${familyPad}px)] sm:pt-[calc(150px+${familyPad}px)]` : 'pt-20';
...
<div class="flex flex-col w-full overflow-hidden relative">
  ...
  <section id="countdownSection" class="${countdownPaddingTop} pb-20 px-margin-mobile text-center relative max-w-lg mx-auto gsap-fade-up overflow-visible">
  ...
  <!-- ==================== 3. FAMILIA & PADRINOS ==================== -->
  <!-- overflow-visible so the bridge illustration can overlap the next section -->
  <section id="family" class="${familyPaddingTop} px-margin-mobile bg-emerald-dark text-center border-y border-antique-gold/30 relative" style="overflow: visible;">
...
<!-- ==================== 10.5 TARJETA DE CONTRATACIÓN & WHATSAPP ==================== -->
<section id="vendorCard" class="py-14 px-margin-mobile bg-emerald-dark/95 text-center relative border-t border-antique-gold/30 ${(vendorCard.enabled !== false) ? '' : 'hidden'}">
  <div class="max-w-md mx-auto relative z-10 gsap-fade-up">
    <div class="p-6 sm:p-8 rounded-2xl border border-antique-gold/50 bg-white/10 backdrop-blur-md shadow-2xl relative overflow-hidden">
      
      <!-- Icono e Insignia -->
      <div class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-antique-gold/20 border border-antique-gold/60 text-antique-gold mb-3 shadow-inner">
        <span class="material-symbols-outlined text-[24px]">mark_email_read</span>
      </div>

      <p class="font-label-caps text-[10px] sm:text-[11px] text-antique-gold tracking-[0.3em] uppercase mb-2 font-semibold" id="vendorCardBadge">
        ${vendorCard.badge || '¿Te gustó esta invitación?'}
      </p>

      <h3 class="font-display-lg text-xl sm:text-2xl text-[#f7f6ec] mb-3 leading-snug" id="vendorCardTitle">
        ${vendorCard.title || 'Diseña tu Invitación Digital de Lujo'}
      </h3>

      <p class="font-body-lg text-xs sm:text-sm text-[#f7f6ec]/85 leading-relaxed mb-6" id="vendorCardDesc">
        ${vendorCard.description || 'Sorprende a tus invitados con una experiencia interactiva inolvidable para Bodas, XV Años y Eventos Exclusivos.'}
      </p>

      <!-- Botón WhatsApp -->
      <a id="btnVendorWhatsapp" href="${vendorWaLink}" target="_blank" rel="noopener noreferrer" class="btn-luxury-primary w-full py-3.5 px-6 text-xs tracking-[0.2em] inline-flex items-center justify-center gap-2 shadow-[0_4px_20px_rgba(165,110,14,0.45)] hover:scale-[1.02] transition-transform">
      ...
      <p class="font-label-caps text-[9px] text-[#f7f6ec]/50 mt-4 tracking-widest uppercase" id="vendorCardAgency">
