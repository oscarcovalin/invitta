...
<div class="flex flex-col w-full overflow-hidden relative">
  ...
  <section id="galeria" class="py-16 px-margin-mobile max-w-xl mx-auto relative overflow-hidden ${isGalleryEnabled ? '' : 'hidden'}">
    ...
    <div class="relative z-10 w-full">
      ...
      <!-- Galería Editorial Rectangular Sin Esquinas Redondeadas, Fotos Completas y Efecto Parallax Visible -->
      <div class="w-full rounded-none overflow-hidden shadow-[0_15px_45px_rgba(0,0,0,0.12)] border border-antique-gold/40 flex flex-col gap-0 bg-champagne-cream/20" id="galleryGrid">
        ${((config.photos && config.photos.gallery) || []).map((url, i) => url ? `
          <div class="gallery-item-wrap w-full relative overflow-hidden rounded-none border-b border-antique-gold/25 last:border-b-0 bg-black/5">
            <img src="${url}" alt="Recuerdo ${i+1}" loading="lazy" class="gallery-photo w-full h-auto block object-contain rounded-none will-change-transform transition-transform duration-700 hover:scale-[1.01]"/>
          </div>
        ` : '').join('')}
      </div>
    </div>
...
document.addEventListener("DOMContentLoaded", () => {
  if(typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined'){
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // PARALLAX EN GALERÍA DE RECUERDOS (Efecto de Desplazamiento Fluido y Profundidad)
    // ─────────────────────────────────────────────────────────────────────────
