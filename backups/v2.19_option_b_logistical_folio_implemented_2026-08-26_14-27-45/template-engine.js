...
if(rsvpForm){
  rsvpForm.addEventListener('submit', (e) => {
    ...
    // Folio Logístico Opción B: [MESA]-[APELLIDO]-[PASES] (ej: M03-MARTINEZ-4P o MIMP-ORTIZ-2P)
    const rawNombre = (payload.nombre || '').trim();
    const cleanTokens = rawNombre
    ...
    const apellidoCode = cleanTokens.length > 0 ? cleanTokens[0].toUpperCase().substring(0, 10) : 'INVITADO';
        ...
        if (typeof localStorage !== 'undefined') {
          ...
          if (rawDb) {
            ...
            if (db && Array.isArray(db.guests)) {
              const cleanN = rawNombre.toLowerCase().replace(/[^a-z0-9]/g, '');
              ...
              if (found && found.tableId) {
                if (found.tableId === 'tbl_imperial' || String(found.tableId).includes('imp')) {
                  mesaCode = 'MIMP';
                } else {
    ...
    const folio = \`\${mesaCode}-\${apellidoCode}-\${payload.pases || 1}P\`;
