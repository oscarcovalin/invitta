    ...
    <div class="relative z-10 max-w-lg mx-auto gsap-scale-up overflow-visible">
      ...
      <div class="pergamino-container text-deep-onyx" id="pergaminoCard">
        ...
        <form id="rsvpForm" class="flex flex-col gap-4 text-left" novalidate>
          ...
          <div id="rsvpTicketsRow" class="flex items-center justify-between p-3.5 bg-champagne-cream/70 border border-antique-gold/40 rounded-xl mt-1">
            <div>
          ...
          <div id="rsvpDietaryBlock" class="flex flex-col gap-2 pt-2 border-t border-antique-gold/25 mt-1">
            <div class="flex items-center justify-between">
              <label class="font-label-caps text-[10.5px] text-deep-onyx uppercase tracking-[0.2em] font-semibold flex items-center gap-1">
                <span>🍽️</span> Preferencias Dietarias (Opcional)
              </label>
              <span class="text-[10px] text-antique-gold/90 font-medium">Para banquetes</span>
            </div>
            ...
            <input type="text" name="alergias" id="formGuestAllergies" placeholder="Alergias o restricciones médicas (ej: nueces, mariscos...)" 
          ...
          <!-- Botón de Envío -->
          <button type="submit" id="rsvpSubmit" class="btn-luxury-primary w-full py-3.5 text-[11.5px] tracking-[0.25em] uppercase font-semibold shadow-[0_4px_20px_rgba(165,110,14,0.35)] mt-2 cursor-pointer transition-all hover:scale-[1.01] active:scale-[0.99]">
...
(function initRsvpPergamino() {
  ...
  if (urlGuest && urlGuest.trim()) {
    let mesaLabelTxt = '';
    if (urlMesaParam) {
      mesaLabelTxt = urlMesaParam.toLowerCase().includes('imp') ? ' en la <strong>Mesa Imperial</strong>' : (' en la <strong>Mesa ' + urlMesaParam.replace(/\\D/g, '') + '</strong>');
    }

    if (saludoCont) {
      saludoCont.innerHTML = '<div class="mensaje-personalizado-pergamino">' +
        '<span class="text-base block mb-0.5">⚜️</span>' +
        '¡Qué alegría tenerles aquí, <strong class="text-deep-onyx">' + urlGuest + '</strong>!<br>' +
        'Tienen <strong class="text-deep-onyx">' + urlTickets + (urlTickets === 1 ? ' pase reservado' : ' pases reservados') + '</strong>' +
        mesaLabelTxt + ' para celebrar nuestro gran día.' +
        '</div>';
    }
    ...
    if (saludoCont) {
      saludoCont.innerHTML = '<div class="mensaje-personalizado-pergamino">' +
        'Por favor ingresa tus datos para registrar tu confirmación de asistencia en el sistema oficial.' +
        '</div>';
    }
