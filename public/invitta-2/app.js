...
function setupHeaderActions() {
  ...
  document.getElementById('btnExportJson').addEventListener('click', async () => {
    ...
    if (window.InvittaCloudBridge && typeof window.InvittaCloudBridge.save === 'function') {
      const handledByCloud = await window.InvittaCloudBridge.save(projectData);
...
window.InvittaStudio = Object.freeze({
  getState() {
    return {
  ...
  applyProject(project) {
    if (!project || !project.config || typeof project.config !== 'object' || Array.isArray(project.config)) {
      throw new Error('El proyecto no contiene una configuración válida.');
    }

    currentConfig = JSON.parse(JSON.stringify(project.config));
    currentThemeName = project.themeName || 'vino';
    customTheme = project.customTheme && typeof project.customTheme === 'object'
      ? JSON.parse(JSON.stringify(project.customTheme))
      : JSON.parse(JSON.stringify(TemplateEngine.defaultThemes.vino));
    currentProjectId = project.projectId || null;

    document.querySelectorAll('.theme-card, .theme-visual-card').forEach(card => {
      card.classList.toggle('selected', card.dataset.theme === currentThemeName);
    });
    const customColors = document.getElementById('customColorsWrapper');
    if (customColors) customColors.style.display = currentThemeName === 'custom' ? 'block' : 'none';
