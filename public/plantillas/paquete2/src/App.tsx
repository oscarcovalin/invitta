...
export default function App() {
  ...
  const getWhatsAppUrl = (
    ...
    if (nameVal) {
      ...
      if (attendingVal === true) {
        ...
        text += `Te agradezco de corazón la hermosa invitación. Lamentablemente *NO podré asistir* en esta ocasión. Te deseo una noche de ensueño llena de magia y momentos inolvidables. ✨💖\n`;
      }
      if (msgVal) {
        text += `\n💬 Mensaje especial:\n"${msgVal}"`;
      }
  ...
  const handleWhatsAppRsvp = (e: React.MouseEvent) => {
    ...
    const whatsappUrl = getWhatsAppUrl(newRsvp.name, newRsvp.attending, newRsvp.guestsCount, newRsvp.dietaryNotes, newRsvp.message);
  ...
  const registries: GiftRegistryOption[] = [
    {
      ...
      name: "Liverpool",
      icon: "card_giftcard",
      details: "Agradezco enormemente tu cariño. Mi mesa de regalos está registrada en Liverpool bajo el número de evento: 51239845 o mi nombre completo.",
  ...
  return (
    <>
      <AnimatePresence mode="wait">
        {!isOpened && (
          <motion.div
            ...
            className="fixed inset-0 z-[100] bg-[#1a1514] flex items-center justify-center overflow-hidden p-4 select-none"
          >
            {/* Fine line border for luxury feel */}
            <div className="absolute inset-4 md:inset-8 border border-sage/20 pointer-events-none z-10"></div>
            
            {/* Corner ornaments */}
            <div className="absolute top-6 left-6 w-8 h-8 border-t border-l border-champagne-gold/40 z-10"></div>
            <div className="absolute top-6 right-6 w-8 h-8 border-t border-r border-champagne-gold/40 z-10"></div>
            <div className="absolute bottom-6 left-6 w-8 h-8 border-b border-l border-champagne-gold/40 z-10"></div>
            <div className="absolute bottom-6 right-6 w-8 h-8 border-b border-r border-champagne-gold/40 z-10"></div>

            {/* Subtle elegant glowing background vector lines */}
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(171,132,120,0.1)_0%,transparent_75%)] pointer-events-none"></div>

            {/* Main Entrance Content Card */}
            <motion.div 
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              transition={{ duration: 1.6, ease: [0.16, 1, 0.3, 1] }}
              className="max-w-md w-full text-center space-y-12 relative px-6 py-14 bg-[#261e1c]/95 border border-sage/10 backdrop-blur-md rounded-xs"
            >
              {/* Subtle top text */}
              <div className="space-y-2">
                <span className="text-[10px] uppercase tracking-[0.4em] text-sage/70 font-semibold block">
                  Invitación de Gala
                </span>
                <div className="w-10 h-px bg-champagne-gold/35 mx-auto"></div>
              </div>

              {/* Main Quinceañera Name */}
              ...
              {/* Deluxe Golden Wax Seal */}
              <motion.div 
                className="relative w-32 h-32 mx-auto flex items-center justify-center cursor-pointer group"
              ...
              >
                ...
                <div className="absolute inset-0 rounded-full border border-champagne-gold/20 group-hover:scale-110 transition-transform duration-700 animate-ping opacity-25"></div>
                ...
                {/* Micro instructions under seal */}
                <span className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-[9px] tracking-[0.25em] text-champagne-gold/70 font-semibold uppercase whitespace-nowrap group-hover:text-champagne-gold transition-colors">
                  Toca para Abrir
                </span>
              </motion.div>
              ...
              <div className="pt-8">
                <button
                  onClick={handleOpenInvitation}
                  className="px-8 py-3.5 border border-champagne-gold/30 text-champagne-gold hover:bg-champagne-gold hover:text-ink text-[11px] tracking-[0.3em] font-semibold uppercase transition-all duration-500 rounded-xs bg-transparent cursor-pointer shadow-lg hover:shadow-champagne-gold/10"
                >
                  ABRIR INVITACIÓN
                </button>
              </div>
            ...
            {/* Decorative classic phrase at bottom */}
            <div className="absolute bottom-12 left-0 right-0 text-center px-4">
              <p className="font-serif italic text-xs text-sage/50 max-w-xs mx-auto">
                "Hay momentos que son inolvidables..."
              </p>
            </div>
          </motion.div>
        )}
      ...
      <div className="bg-paper min-h-screen text-ink flex flex-col font-sans selection:bg-sage/20 relative">
        ...
        {invData.musicUrl && <MusicPlayer autoPlay={isOpened} />}
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          >
            ...
            <div className="pt-8 text-center space-y-3">
              <span className="text-subheading-caps text-[9px] text-sage tracking-[0.3em] block">Mis Quince Años</span>
      ...
      <main className="w-full overflow-hidden flex-grow">
        ...
        <section id="hero" className="relative min-h-screen flex flex-col md:flex-row items-center pt-24 md:pt-0">
          ...
          <motion.div 
            ...
            className="w-full md:w-7/12 h-[65vh] md:h-screen mt-12 md:mt-0 relative overflow-hidden group"
          >
            <motion.div 
              style={{ y: yHero }}
              className="absolute inset-0 h-[115%] -top-[7%] w-full"
            >
              <img 
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-paper via-transparent to-transparent md:block hidden opacity-60 pointer-events-none"></div>
            <div className="absolute inset-0 bg-gradient-to-t from-paper via-transparent to-transparent md:hidden block opacity-90 h-32 bottom-0 top-auto pointer-events-none"></div>
          </motion.div>
        </section>

        {/* FAMILY HONORS SECTION */}
        <section id="honors" className="py-section-gap px-margin-mobile bg-white/40">
              ...
              >
                <div className="space-y-6 p-8 md:p-12 bg-surface-container-low/60 border border-outline-variant/15 relative">
                  ...
                  <p className="text-xs text-on-surface-variant font-light italic leading-relaxed pt-2">
                    "Honrada de contar con su gran compañía y apoyo constante en esta noche mágica."
                  </p>
                </div>
              </motion.div>
            </div>
        ...
        {/* COUNTDOWN BANNER */}
        <section id="countdown" className="relative py-32 bg-ink text-paper overflow-hidden">
        ...
        {/* DRESS CODE SECTION */}
        <section className="relative py-section-gap bg-ink text-paper flex flex-col md:flex-row items-center min-h-[70vh]">
          ...
          >
            ...
            <div className="space-y-8 pl-0 md:pl-8 border-l-0 md:border-l border-sage/40">
              <p className="font-display text-lg md:text-xl italic text-paper/85 font-light leading-relaxed max-w-md mx-auto md:mx-0">
        ...
        <section id="registry" className="py-section-gap px-margin-mobile bg-surface-container-low/20">
          <div className="max-w-6xl mx-auto space-y-16">
            ...
            >
              ...
              <p className="text-[#454841] text-sm md:text-base font-light max-w-2xl mx-auto font-sans leading-relaxed">
                Su grata compañía el día de mi evento es mi mayor regalo, pero si desea tener un detalle de celebración especial conmigo, aquí tiene mis sugerencias.
              </p>
              ...
              {registries.map((reg, i) => (
                ...
                >
                  <div className="space-y-4">
                    <span className="material-symbols-outlined text-4xl text-sage/75 group-hover:text-sage transition-colors duration-300">
                      {reg.icon}
                    </span>
        ...
        <section id="gallery" className="py-section-gap px-margin-mobile max-w-[1600px] mx-auto bg-white/30">
        ...
        <section id="itinerary" className="py-28 px-margin-mobile bg-[#fffcfb] relative overflow-hidden border-t border-b border-outline-variant/10">
          ...
          <div className="absolute top-0 left-0 w-48 h-48 md:w-64 md:h-64 pointer-events-none select-none opacity-40">
            <svg viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
              ...
              <path d="M10 10 C 30 50, 70 60, 110 50 C 90 90, 50 110, 10 100 Z" fill="#ab8478" opacity="0.15" />
              <path d="M10 10 C 50 30, 60 70, 50 110 C 90 90, 110 50, 10 10 Z" fill="#dfba6b" opacity="0.12" />
              <path d="M10 10 C 60 10, 100 30, 120 70 C 80 80, 40 60, 10 10 Z" fill="#ab8478" opacity="0.18" />
              {/* Delicate cream rose shapes */}
              <circle cx="35" cy="35" r="25" fill="#fcf3f0" opacity="0.75" />
              <circle cx="35" cy="35" r="18" fill="#fadcd4" opacity="0.65" />
              <circle cx="30" cy="30" r="12" fill="#e8b4a8" opacity="0.5" />
              <circle cx="45" cy="45" r="15" fill="#fcf3f0" opacity="0.65" />
              {/* Elegant fine lines for branches */}
              <path d="M0 0 Q 60 40 130 60" stroke="#ab8478" strokeWidth="0.75" opacity="0.3" />
              <path d="M0 0 Q 40 60 70 140" stroke="#ab8478" strokeWidth="0.75" opacity="0.3" />
              <path d="M0 0 Q 80 20 150 20" stroke="#ab8478" strokeWidth="0.5" opacity="0.25" />
            </svg>
          </div>
            ...
            {rsvpSubmitted ? (
              <div className="space-y-8 animate-fadeIn mt-12 text-center">
                ...
                >
                  ...
                  <p className="font-sans text-[13px] md:text-sm text-[#454841] leading-relaxed max-w-xl mx-auto">
                    {rsvpSubmitted.attending ? (
                      <span>
                        Para mí y para mi familia, tu presencia es el regalo más valioso. Nos llena de inmensa felicidad saber que nos acompañarás a celebrar mis Quince Años en esta noche mágica. Prepárate para una velada de ensueño llena de luz, música y recuerdos inolvidables. ¡Nos vemos muy pronto!
                      </span>
                    ) : (
                      <span>
                ...
                <div className="p-8 border border-champagne-gold/35 bg-[#faf9f7] space-y-8 text-center relative overflow-hidden rounded-xs">
                  ...
                  <div className="border-t border-b border-outline-variant/20 py-6 space-y-4 text-left text-xs text-[#454841] leading-relaxed">
                  ...
                  <div className="pt-4 space-y-4">
                    {/* Decorative Barcode to feel like a real luxurious printed ticket */}
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full flex justify-around items-center py-4 bg-paper/90 backdrop-blur-md z-40 border-t border-outline-variant/15 shadow-lg">
        ...
        <button 
          onClick={() => scrollToSection("details")}
          className="flex flex-col items-center justify-center text-[#454841]/70 hover:text-sage transition-colors gap-1 cursor-pointer"
        >
