...
export function RsvpAdmin({ isOpen, onClose, rsvps, onDeleteRsvp, onClearAll }: RsvpAdminProps) {
  ...
  const exportToCsv = () => {
    ...
    link.setAttribute("download", `RSVP_Boda_Ana_Camila_y_Carlos_2026.csv`);
      ...
      const samples: RSVP[] = [
        {
          ...
          message: "¡Muchas felicidades Ana Camila y Carlos, nos vemos pronto!",
          tableNumber: "Mesa 4"
        },
        {
          id: "2",
  ...
  const handleUpdateTableNumber = (id: string, currentTable: string) => {
    const newTable = prompt(`Asignar/Cambiar Mesa para este invitado:`, currentTable || "");
    if (newTable !== null) {
      try {
        const itemStr = localStorage.getItem(id);
        if (itemStr) {
          const item = JSON.parse(itemStr);
          item.tableNumber = newTable.trim();
          localStorage.setItem(id, JSON.stringify(item));
          window.location.reload();
        } else {
          // If it's a sample item without premium prefix or is direct
          const item = rsvps.find(r => r.id === id);
          if (item) {
            const updated = { ...item, tableNumber: newTable.trim() };
            localStorage.setItem(`rsvp_${id}`, JSON.stringify(updated));
            window.location.reload();
          }
        }
      } catch (e) {
        console.error("Error updating table number", e);
      }
    }
  };
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-surface-container-low/75 border-b border-outline-variant/20">
                      ...
                      <th className="p-4 text-[10px] font-semibold tracking-widest text-sage uppercase text-center">Mesa</th>
                    ...
                    {filteredRsvps.map((rsvp) => (
                      <tr key={rsvp.id} className="hover:bg-paper/50 transition-colors">
                        ...
                        <td className="p-4 text-center">
                          {rsvp.attending ? (
                            <button
                              onClick={() => handleUpdateTableNumber(rsvp.id, rsvp.tableNumber || "")}
                              className="px-2.5 py-1 border border-sage/20 hover:border-sage rounded-xs text-sage hover:bg-sage/5 transition-all text-[11px] font-medium whitespace-nowrap cursor-pointer select-none"
                              title="Asignar o editar número de mesa"
                            >
                              {rsvp.tableNumber || "Asignar Mesa"}
                            </button>
                          ) : (
