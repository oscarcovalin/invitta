...
export function DigitalEnvelope({ onOpen }: DigitalEnvelopeProps) {
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-[120] bg-[#0f111e] flex items-center justify-center overflow-hidden p-4 select-none"
    >
      {/* Elegantly textured background with a soft ambient radial gold spotlight */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(234,200,139,0.1)_0%,transparent_70%)] pointer-events-none"></div>
      ...
      {/* Frame border */}
      <div className="absolute inset-4 md:inset-8 border border-champagne-gold/10 pointer-events-none z-10"></div>
      ...
      <div className="relative w-full max-w-lg aspect-[4/3] md:aspect-[1.414/1] flex items-center justify-center pt-12 md:pt-16">
        ...
        <div className="relative w-[90%] md:w-[85%] h-[68%] md:h-[72%] [perspective:1200px]">
          ...
          <motion.div
            ...
            className="absolute left-[3%] right-[3%] top-[4%] bottom-[4%] bg-[#1c2038] rounded-sm p-5 md:p-8 flex flex-col justify-between shadow-[0_15px_40px_rgba(0,0,0,0.4)] border border-champagne-gold/25 z-[15]"
          >
            {/* Fine internal border for premium stationary feel */}
            <div className="absolute inset-2 md:inset-3 border border-champagne-gold/15 rounded-xs pointer-events-none"></div>

            {/* Micro ornaments */}
            <div className="absolute top-4 left-4 w-4 h-4 border-t border-l border-champagne-gold/35"></div>
            <div className="absolute top-4 right-4 w-4 h-4 border-t border-r border-champagne-gold/35"></div>
            <div className="absolute bottom-4 left-4 w-4 h-4 border-b border-l border-champagne-gold/35"></div>
            <div className="absolute bottom-4 right-4 w-4 h-4 border-b border-r border-champagne-gold/35"></div>

            <div className="text-center space-y-2 md:space-y-3 my-auto">
              <span className="text-[8px] md:text-[9px] uppercase tracking-[0.4em] text-champagne-gold/80 font-semibold block">
                Invitación de Gala
              </span>
              <div className="w-8 h-[1px] bg-champagne-gold/35 mx-auto"></div>
              
              <h3 className="font-serif italic text-2xl md:text-3xl text-champagne-gold block select-none">
                Nuestra Boda
              </h3>
              
              <h2 className="font-serif text-2xl md:text-3xl text-ink tracking-wide font-light">
                Ana Camila &amp; Carlos
              </h2>
              
              <p className="font-sans text-[9px] md:text-[10px] uppercase tracking-[0.3em] text-champagne-gold/90">
                Zavala &amp; González
              </p>
              
              <div className="pt-2 md:pt-4">
                <p className="font-serif italic text-[11px] text-on-surface-variant max-w-[240px] mx-auto leading-relaxed">
          ...
          <div className="absolute inset-0 bg-[#121422] rounded-sm border border-[#222744] shadow-xl z-10 overflow-hidden">
            {/* Elegant luxury pattern inside the envelope pocket */}
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(234,200,139,0.12)_0%,transparent_80%)]"></div>
            <div className="absolute inset-0 opacity-[0.03] bg-[linear-gradient(45deg,#eac88b_25%,transparent_25%,transparent_75%,#eac88b_75%,#eac88b),linear-gradient(45deg,#eac88b_25%,transparent_25%,transparent_75%,#eac88b_75%,#eac88b)] bg-[size:16px_16px] bg-[position:0_0,8px_8px]"></div>
          </div>
          ...
          <div className="absolute inset-0 pointer-events-none z-[18] flex items-end">
            <svg viewBox="0 0 500 300" className="w-full h-[60%] filter drop-shadow-[0_-5px_10px_rgba(0,0,0,0.15)] overflow-visible">
              {/* Left and Right overlapping panels styled elegant deep velvet plum */}
              <path d="M 0 300 L 250 145 L 500 300 Z" fill="#1c2038" stroke="#2a3054" strokeWidth="1.5" />
              <path d="M 0 0 L 195 160 L 0 300 Z" fill="#121422" opacity="0.9" />
              <path d="M 500 0 L 305 160 L 500 300 Z" fill="#121422" opacity="0.9" />
            </svg>
          </div>
          ...
          >
            <svg viewBox="0 0 500 160" className="w-full h-full filter drop-shadow-[0_8px_15px_rgba(0,0,0,0.25)]">
              ...
              <path d="M 0 0 L 250 160 L 500 0 Z" fill="#222744" stroke="#313861" strokeWidth="1.5" />
            </svg>
          </motion.div>
          ...
          >
            <svg viewBox="0 0 500 160" className="w-full h-full transform scale-y-[-1] opacity-90">
              <path d="M 0 0 L 250 160 L 500 0 Z" fill="#121422" stroke="#1c2038" strokeWidth="1" />
            </svg>
          </motion.div>
